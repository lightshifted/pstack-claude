...
Locate the active workspace's transcripts before fanning out. Transcripts for this project live in `~/.claude/projects/<slug>/`, where `<slug>` is the project path with the leading slash dropped and every "/" turned into "-". Use only that path. Don't glob across `~/.claude/projects/*/`. That crosses workspace boundaries and reads private chats from unrelated projects.
