...
Before spawning investigators, list the MCP servers actually connected in this session. Your own tool list is the dependable source: MCP tools are named `mcp__<server>__<tool>`. When a tool you need is deferred, load its schema with `ToolSearch` before you fan out, so an investigator is never handed a tool it cannot call.
...
- `model`: omit. The agent definition carries the model and the effort. Pass `model` only to force a different family for one call.
- Tool access: leave it alone. The agent withholds `Edit`, `Write`, and `NotebookEdit` and nothing else, so every MCP server you can reach, the investigator can reach. **Never narrow it with a `tools` allowlist.** An allowlist cannot name the user's MCP tools, and an MCP-backed investigator without them is this whole skill defeated.
...
- `subagent_type`: `poteto-explainer`
- `model`: omit. The agent definition carries the model and the effort. Pass `model` only to force a different family for one call.
- Tool access: leave it alone, for the same reason. The synthesizer's quality check spot-verifies citations, which can require the MCP servers the investigators used.
