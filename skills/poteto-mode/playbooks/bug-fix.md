...
3. Plan the fix. If it crosses a function boundary, `architect` first. Delegate implementation to a `poteto-code` subagent (or `poteto-judge` when the fix is subtle) with a specific scope. Review the diff.
