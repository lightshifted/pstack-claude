...
Each live lane runs on its own remote worktree at the PR head. Drive through the surface driver named in **Driving the real surface**.
...
**Verify, live.** Tests alone are not sufficient verification. A PR is verified only when its unit, live, and perf boxes are all checked. Ten lanes on `poteto-explorer` at the PR head, per the boot recipe.
