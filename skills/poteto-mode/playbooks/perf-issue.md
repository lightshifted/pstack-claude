...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Delegate implementation to a `poteto-code` subagent (or `poteto-judge` when the fix is subtle). Review the diff. Capture a post-fix trace.
